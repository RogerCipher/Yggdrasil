...
int insertInWeightedList(TipoNode **cabeca, TipoNode *elemento)
{
    ...
    while(iterador != NULL) {
        ...
        if (iterador->weight > elemento->weight) { // Insert before
            elemento->right = iterador;
            elemento->left = iterador->left;
            iterador->left = elemento;
            if (elemento->left == NULL) {
                *cabeca = elemento;
            } else {
                elemento->left->right = elemento;
            }
            return 1;
        }
        iterador = iterador->right;
...
void printHuffTree(TipoNode *elemento)
{
    ...
    if (elemento->left != NULL) 
    {
        ...
        printHuffTree(elemento->left);    // imprimir primeiro filho

        if(elemento->right != NULL)
            printHuffTree(elemento->right);
    }
    if (elemento->parent == NULL) {
        //fim
        printf("}\n");
